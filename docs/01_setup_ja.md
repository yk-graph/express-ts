...
- [x] `.prettierrc` の作成（Prettier のフォーマットルールを一元管理）
...
| `prettier/prettier: 'error'` | Prettier のフォーマット違反を ESLint のエラーとして検出する。フォーマットの詳細は `.prettierrc` で一元管理する。 |
| `eslintPluginPrettierRecommended` | Prettier のフォーマットと競合する ESLint ルールを無効にする。カスタムルールより前に置くことで、後勝ちルールにより上書きされる。 |
...
| `singleQuote: true` | 文字列をシングルクォート `'` で統一する（デフォルトはダブルクォート）。 |
| `printWidth: 200` | 1行の最大文字数を200に設定する（デフォルトは80）。 |
| `htmlWhitespaceSensitivity: 'strict'` | HTML の空白をユーザーが入力した通りに保持する。 |
| `endOfLine: 'lf'` | 改行コードを LF に統一する。CRLF（Windows）との混在を防ぐ。 |

> **なぜ `.prettierrc` を別ファイルにするのか？**
> Prettier の設定を `eslint.config.mjs` にインラインで書くと、ESLint のルールとしてしか機能しない。VS Code の Prettier 拡張は `eslint.config.mjs` を読まず、`.prettierrc` を探して読む。両者を一致させるために `.prettierrc` で一元管理する。
...
| `.prettierrc` | Prettier のフォーマットルール。VS Code 拡張と ESLint の両方が参照する。 |
